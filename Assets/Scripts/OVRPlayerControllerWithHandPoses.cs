/* This Script is a modified version of the OVRPlayerController Script, it has been modified to include the hand poses for the Oculus Quest 2,
   This script was modified by dilmer valecillos, the original script can be found here: https://github.com/dilmerv/MetaInteractionSDKDemos/blob/master/Assets/Scripts/OVRPlayerControllerWithHandPoses.cs
   Some comments in the script were created by Shane Cunningham, they are marked with a double forward slash (//) at the beginning of the comment.
   The comments will indicate the changes made to the original script, they are located in the script at the following lines: 155, 187, 364, 370, 473, 482, 501, 504 
 */
...
public class OVRPlayerControllerWithHandPoses : MonoBehaviour
{
    /// <summary>
    ...
    // The following code section is included for gesture control, these are the changes made to the original OVRPlayerController script
    // They are included to allow the user control their movement using hand gestures instead of using joysticks
    ...
    public void SetHandPoseMoveForward(bool state)
    {
        ...
        //Debug.Log(state); // Test to see if null reference exception error is coming from here
    ...
    void Start()
    {
        ...
        CameraRig = GetComponent<OVRCameraRig>();  // Included for custom gestures
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (EnableLinearMovement)
        {
            ...
            if (OVRInput.Get(OVRInput.Button.DpadUp) || HandPoseMoveForward) // This is Included for gesture control
            {
                moveForward = true;
                dpad_move = true;
            }

            if (OVRInput.Get(OVRInput.Button.DpadDown) || HandPoseMoveBackward) // This is Included for gesture control
            {
                moveBack = true;
                dpad_move = true;
            }
        ...
        if (EnableRotation)
        {
            ...
            if (SnapRotation)
            {
                if (OVRInput.Get(OVRInput.Button.SecondaryThumbstickLeft) ||
                    (RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.PrimaryThumbstickLeft)) || HandPoseRotateLeft) // This is Included for gesture control
                {
                    if (ReadyToSnapTurn)
                    {
                        euler.y -= RotationRatchet;
                    ...
                    (RotationEitherThumbstick && OVRInput.Get(OVRInput.Button.PrimaryThumbstickRight)) || HandPoseRotateRight) // This is Included for gesture control
                {
                    if (ReadyToSnapTurn)
                    {
                        euler.y += RotationRatchet;
            ...
            else
            {
                ...
                if (RotationEitherThumbstick)
                {
                    ...
                    if (secondaryAxis.sqrMagnitude < altSecondaryAxis.sqrMagnitude || HandPoseRotateLeft || HandPoseRotateRight) // This is Included for gesture control
                    {
                        Vector2 handPoseRotation = HandPoseRotateLeft ? Vector2.left : Vector2.right;
                        secondaryAxis = (HandPoseRotateLeft || HandPoseRotateRight) ? handPoseRotation : altSecondaryAxis; // This is Included for gesture control
                    }
                }
